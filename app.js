...
// Helper function to call Cloud Functions with explicit auth token
async function callCloudFunctionWithAuth(functionName, data) {
    console.log(`[Cloud Function] Calling ${functionName}...`);
    ...
    console.log(`[Cloud Function] Got auth token, length: ${idToken.length}`);
    ...
    console.log(`[Cloud Function] Sending data:`, data);
    ...
    try {
        ...
        console.log(`[Cloud Function] Success:`, responseData);
                        ...
                        if (userDoc.exists) {
                            ...
                            if (app.userRole === 'restaurant_owner') {
                                ...
                                if (!restaurantDoc.empty) {
                                    ...
                                    // Check payment status before showing dashboard
                                    const paymentStatus = await checkPaymentStatus(app.currentRestaurantId, user.uid);
                                    
                                    if (paymentStatus.isPaymentConfirmed && paymentStatus.isSubscriptionActive) {
                                        // Payment confirmed and subscription active - show dashboard button
                                        showDashboardButton();
                                    } else if (paymentStatus.isPending) {
                                        // Payment pending - show payment pending message
                                        showPaymentPendingButton();
                                    } else {
                                        // No payment or expired subscription - show payment required message
                                        showPaymentRequiredButton();
                                    }
                                } else {
...
function showPaymentRequiredButton() {
    ...
    const required = heroBtns.querySelector('[onclick*="payment"]');
    if (!required) {
        const btn = document.createElement('button');
        btn.className = 'btn btn-primary';
        btn.textContent = '💳 Complete Payment (₹999)';
        btn.onclick = () => showPaymentRequiredModal();
        heroBtns.appendChild(btn);
    }
}
...
function showPaymentRequiredModal() {
    ...
    modal.innerHTML = `
        <div style="background: white; border-radius: 12px; padding: 30px; max-width: 400px; text-align: center;">
            <h2 style="margin-top: 0; color: #0F172A;">Payment Required</h2>
            <p style="color: #64748B;">Your restaurant is ready! Complete the payment to unlock all features.</p>
            <div style="background: #F8FAFC; padding: 20px; border-radius: 8px; margin: 20px 0;">
                ...
                <p style="margin: 10px 0 0 0; font-size: 12px; color: #64748B;">+ ₹499/month subscription</p>
            </div>
            <button onclick="location.reload()" style="background: #EF4444; color: white; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; width: 100%; font-size: 16px; font-weight: 600;">Contact Support</button>
            <button onclick="this.closest('div').parentElement.remove()" style="background: #E2E8F0; color: #0F172A; border: none; padding: 12px 24px; border-radius: 6px; cursor: pointer; width: 100%; margin-top: 10px; font-size: 14px;">Close</button>
        </div>
    `;
...
function navigateTo(page) {
    ...
    if (page === 'landing') {
        ...
        // Check payment status before allowing dashboard access
        if (app.currentRestaurantId && app.currentUser) {
            checkPaymentStatus(app.currentRestaurantId, app.currentUser.uid).then(paymentStatus => {
                if (!paymentStatus.isPaymentConfirmed || !paymentStatus.isSubscriptionActive) {
                    // Payment not confirmed or subscription expired
                    showNotification('⚠️ Payment required to access dashboard. ₹999 one-time + ₹499/month', 'error');
                    showPaymentRequiredModal();
                    navigateTo('landing');
                    return;
                }
                
                // Payment confirmed, show dashboard
...
async function initiatePayment() {
    ...
    try {
        ...
        try {
            ...
            // Use HTTP call with explicit auth token instead of callable function
            const result = await callCloudFunctionWithAuth('initiatePaymentWithCashfree', {
            ...
            // Store order ID and session ID for verification
            app.onboardingData.cashfreeOrderId = result.orderId;
            app.cashfreeSessionId = result.sessionId;
            
            console.log('Cashfree Session ID:', app.cashfreeSessionId);
            console.log('Order ID:', result.orderId);
            
            // Open payment gateway with Cashfree Secure Checkout
            openCashfreePayment(result, restaurantRef.id);
            
        } catch (functionError) {
            console.error('=== PAYMENT FUNCTION ERROR ===');
            console.error('Error:', functionError.message || functionError);
            console.error('Auth current user:', auth.currentUser ? auth.currentUser.uid : 'NULL');
            console.error('========================');
            
            showNotification('Failed to initiate payment: ' + (functionError.message || 'Unknown error'), 'error');
            
            // Clean up - delete the pending restaurant
            if (restaurantRef) {
                try {
                    console.log('Cleaning up restaurant record...');
                    await db.collection('restaurants').doc(restaurantRef.id).delete();
                } catch (deleteError) {
                    console.error('Error cleaning up restaurant:', deleteError);
                }
            }
        }
    } catch (error) {
        ...
        // Clean up failed restaurant creation
        if (restaurantRef) {
            try {
                await db.collection('restaurants').doc(restaurantRef.id).delete();
            } catch (deleteError) {
    ...
    try {
        ...
        paymentContainer.innerHTML = `
            <div style="background: white; border-radius: 12px; padding: 30px; max-width: 500px; width: 90%; text-align: center; box-shadow: 0 10px 40px rgba(0,0,0,0.3);">
                ...
                <button onclick="proceedToCashfreeCheckout('${paymentData.orderId}', '${restaurantId}', ${paymentData.amount})" 
        ...
        console.log('Modal element:', document.getElementById('cf-payment-container'));
...
async function proceedToCashfreeCheckout(orderId, restaurantId, amount) {
    try {
        ...
        console.log('Session ID:', app.cashfreeSessionId);
        
        showNotification('Opening Cashfree Payment Gateway...', 'info');
        
        // Store payment info in session storage for return verification
        sessionStorage.setItem('paymentOrderId', orderId);
        sessionStorage.setItem('paymentRestaurantId', restaurantId);
        sessionStorage.setItem('paymentAmount', amount);
        
        // Check if Cashfree SDK is loaded
        if (typeof Cashfree === 'undefined') {
            console.error('❌ Cashfree SDK not loaded in window');
            console.error('Available window properties:', Object.keys(window).filter(k => k.includes('cash') || k.includes('Cash')).slice(0, 20));
            showNotification('Cashfree SDK not loaded. Please refresh the page.', 'error');
            return;
        }
        ...
        if (!app.cashfreeSessionId) {
            ...
            console.error('app.cashfreeSessionId:', app.cashfreeSessionId);
            showNotification('Payment session not ready. Please try again.', 'error');
            return;
        }

        console.log('✓ Session ID ready:', app.cashfreeSessionId);

        // Prepare checkout options
        const checkoutOptions = {
            paymentSessionId: app.cashfreeSessionId,
            returnUrl: window.location.origin + '/?paymentStatus=success&orderId=' + orderId + '&restaurantId=' + restaurantId,
        ...
        // Use Cashfree's redirect method
        if (typeof Cashfree.checkout === 'function') {
            console.log('Calling Cashfree.checkout()...');
            Cashfree.checkout(checkoutOptions)
                .then(function(response){
                    console.log('✓ Cashfree checkout returned:', response);
                    if (response && response.error) {
                        console.error('❌ Cashfree error:', response.error);
                        showNotification('Payment failed: ' + response.error, 'error');
                    }
                })
            ...
            console.error('Cashfree methods:', Object.keys(Cashfree));
            showNotification('Payment gateway error. Please refresh and try again.', 'error');
        }
...
async function verifyPaymentAndComplete(orderId, restaurantId) {
    ...
    try {
        ...
        console.log('Parameters:', { restaurantId, orderId, amount: 999 });
        
        // Call backend to verify payment with Cashfree
        const result = await functions.httpsCallable('verifyAndCompletePayment')({
