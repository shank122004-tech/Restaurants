...
function showDashboardButton() {
    ...
    // Add setup fee button below dashboard button
    setTimeout(() => {
        checkSetupFeeStatusAndAddButton();
    }, 500);
}
...
function updateMenuTabsVisibility() {
    // Always show all tabs - users need to see them to add items
    const allTabs = document.querySelectorAll('.menu-tabs .tab-btn');
    allTabs.forEach(tab => {
        tab.style.display = 'flex';  // Always visible
    });
}
...
function switchMenuTab(tab) {
    ...
    // Clear session cache for this tab so it reloads fresh data
    // This ensures that newly added items appear immediately
    if (tab === 'foods') {
        app.sessionLoaded.menu = false;
        loadMenuData();
    } else if (tab === 'categories') {
        app.sessionLoaded.categories = false;
        loadCategories();
    } else if (tab === 'variants') {
        app.sessionLoaded.variants = false;
        loadVariants();
    } else if (tab === 'addons') {
        app.sessionLoaded.addons = false;
        loadAddons();
    }
...
function generateQRCode() {
    ...
    const qrLink = 'https://tablepilot.online?restaurant=' + app.currentRestaurantId;
                    ...
                    <div class="card-content">
                        ...
                        <div class="contact-methods">
                            <div class="contact-item">
                                ...
                                <a href="https://wa.me/917982638110" target="_blank" class="contact-link">
                                    <span class="contact-icon">💬</span>
                                    <span>+91 79826 38110</span>
                                </a>
                            </div>
                            
                            <div class="contact-item">
                                <div class="contact-label">Email Support</div>
                                <a href="mailto:tablepilot.online@gmail.com" class="contact-link">
                                    <span class="contact-icon">📧</span>
                                    <span>tablepilot.online@gmail.com</span>
                                </a>
                            </div>
                        </div>
                    </div>
    ...
    const policyData = {
        ...
        privacy: {
            ...
            content: `
                ...
                <h5>6. Your Rights</h5>
                <p>You have the right to access, correct, or delete your personal information. Contact us at tablepilot.online@gmail.com for any requests.</p>
                
                <h5>7. Contact Us</h5>
                <p>For privacy concerns, contact: tablepilot.online@gmail.com or WhatsApp: +91 79826 38110</p>
            `
        },
        refund: {
            title: 'Refund Policy',
            ...
            content: `
                ...
                <p>- Email: tablepilot.online@gmail.com<br/>
                - WhatsApp: +91 79826 38110<br/>
        ...
        printWindow.document.write(`
            ...
            <body>
                ...
                <script>
                    ...
                    const restaurantUrl = 'https://tablepilot.online?restaurant=' + restaurantId;
...
// ============================================
// SETUP FEE PAYMENT FUNCTIONS
// ============================================

function showSetupFeeModal() {
    console.log('[Setup Fee] Opening modal...');
    const modal = document.getElementById('setup-fee-modal');
    if (modal) {
        modal.style.display = 'flex';
        modal.style.visibility = 'visible';
        console.log('[Setup Fee] Modal display set to flex');
        // Populate user email if available
        const emailInput = document.getElementById('setup-fee-email');
        if (auth.currentUser && emailInput) {
            emailInput.value = auth.currentUser.email;
            emailInput.readOnly = true;
        }
        // Update setup fee amount based on country
        updateSetupFeeDisplay();
        console.log('[Setup Fee] Modal opened successfully');
    } else {
        console.error('[Setup Fee] Modal element not found');
    }
}
...
async function updateSetupFeeDisplay() {
    try {
        const currentUser = auth.currentUser;
        ...
        if (restaurantsQuery.empty) return;

        const restaurantData = restaurantsQuery.docs[0].data();
        const country = restaurantData.country || 'India';
        
        const amountDisplay = document.getElementById('setup-fee-amount');
        const countryDisplay = document.getElementById('setup-fee-country');
        
        if (country === 'USA') {
            amountDisplay.textContent = '$25';
            countryDisplay.textContent = 'United States';
        } else if (country === 'UK') {
            amountDisplay.textContent = '£25';
            countryDisplay.textContent = 'United Kingdom';
        } else {
            amountDisplay.textContent = '₹699';
            countryDisplay.textContent = 'India';
        }
    } catch (error) {
        console.error('[Setup Fee] Error updating display:', error);
    }
}

async function initiateSetupFeePayment() {
    try {
        const currentUser = auth.currentUser;
        if (!currentUser) {
            showNotification('Please sign in first', 'error');
        ...
        if (!userEmail || userEmail !== currentUser.email) {
            showNotification('Please enter the email address you signed in with', 'error');
            return;
        }

        const payBtn = document.getElementById('setup-fee-btn-pay');
        ...
        if (restaurantsQuery.empty) {
            showNotification('Restaurant profile not found', 'error');
        ...
        const functionUrl = 'https://us-central1-gurufinder-6fd24.cloudfunctions.net/setupFeePaymentOrder';
        ...
        if (!response.ok || !data.success) {
            showNotification('❌ ' + (data.error || 'Failed to create payment order'), 'error');
            payBtn.disabled = false;
            payBtn.textContent = '💳 Proceed to Payment';
            return;
        }

        console.log('[Setup Fee] Payment order created:', data);
        ...
        try {
            ...
            console.log('[Setup Fee] Checkout result:', result);

            // Verify payment after checkout
            await verifySetupFeePayment(data.orderId, currentUser.uid);

        } catch (checkoutError) {
            console.error('[Setup Fee] Checkout error:', checkoutError);
            showNotification('Payment checkout failed: ' + checkoutError.message, 'error');
            payBtn.disabled = false;
            payBtn.textContent = '💳 Proceed to Payment';
        }
...
async function verifySetupFeePayment(orderId, ownerId) {
    try {
        const verifyUrl = 'https://us-central1-gurufinder-6fd24.cloudfunctions.net/verifySetupFeePayment';
        ...
        const data = await response.json();
        console.log('[Setup Fee] Verification result:', data);

        if (data.success) {
            showNotification('✅ Setup fee payment successful! 🎉', 'success');
            closeSetupFeeModal();
            ...
            showNotification('⚠️ Payment completed but verification pending. Please refresh the page.', 'warning');
            setTimeout(() => {
                window.location.reload();
            }, 3000);
        }
    } catch (error) {
        console.error('[Setup Fee] Verification error:', error);
        showNotification('⚠️ Payment completed. Please refresh to see updates.', 'warning');
    }
}

async function checkSetupFeeStatusAndAddButton() {
    try {
        console.log('[Setup Fee] Checking setup fee status...');
        const currentUser = auth.currentUser;
        if (!currentUser) {
            console.log('[Setup Fee] No current user');
            return;
        }
        ...
        if (restaurantsQuery.empty) {
            console.log('[Setup Fee] No restaurant found');
            return;
        }

        const restaurantData = restaurantsQuery.docs[0].data();
        const setupFeePaid = restaurantData.setupFee?.paid === true;
        
        console.log('[Setup Fee] Status - Paid:', setupFeePaid, 'Country:', restaurantData.country);

        // Remove existing setup fee button if any
        const existingSetupFeeBtn = document.getElementById('hero-setup-fee-btn');
        if (existingSetupFeeBtn) {
            existingSetupFeeBtn.remove();
        }

        // Only add button if setup fee is not paid
        if (!setupFeePaid) {
            const heroBtns = document.querySelector('.hero-buttons');
            if (heroBtns) {
                const country = restaurantData.country || 'India';
                let buttonText = '💳 Pay ₹699 Setup Fee';
                if (country === 'USA') {
                    buttonText = '💳 Pay $25 Setup Fee';
                } else if (country === 'UK') {
                    buttonText = '💳 Pay £25 Setup Fee';
                }

                const setupFeeBtn = document.createElement('button');
                setupFeeBtn.id = 'hero-setup-fee-btn';
                setupFeeBtn.className = 'btn btn-primary btn-lg';
                setupFeeBtn.textContent = buttonText;
                setupFeeBtn.onclick = () => {
                    console.log('[Setup Fee] Button clicked');
                    showSetupFeeModal();
                };
                setupFeeBtn.style.marginTop = '12px';
                setupFeeBtn.style.display = 'block';
                heroBtns.appendChild(setupFeeBtn);
                console.log('[Setup Fee] Button added to hero section:', buttonText);
            } else {
                console.error('[Setup Fee] Hero buttons element not found');
            }
        } else {
            console.log('[Setup Fee] Setup fee already paid');
        }
    } catch (error) {
        console.error('[Setup Fee] Error checking status:', error);
    }
}
