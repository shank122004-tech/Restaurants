...
function setupOrdersListener(filter) {
    ...
    // Guard: If listener exists and filter hasn't changed, don't recreate
    if (app.ordersListener && app.currentOrdersFilter === filter) {
        console.log('[Orders] Listener already active for filter:', filter);
        return;
    }
    
    // Unsubscribe from previous listener only if filter changed
...
function setupKitchenListener() {
    ...
    if (app.kitchenListener && app.kitchenListenerActive) {
    ...
    app.kitchenListenerActive = true;
