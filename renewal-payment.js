 ...
 * Optimized for reduced network calls and improved performance
 */
...
(function() {
  ...
  // ────── LAZY-LOAD CASHFREE SDK - CACHED ──────
  function loadCashfreeSDK() {
    // Return immediately if already loaded
    if (_cfSdkLoaded && typeof Cashfree === 'function') {
    ...
    if (_cfSdkLoading) {
      console.log('[renewal] Cashfree SDK is already loading');
  ...
  // ────── PAYPAL RENEWAL HANDLER (USA/UK) - PAYPAL.ME ──────
  window.handlePayPalRenewal = async function(currency = 'USD') {
    try {
      console.log('[renewal] ===== PAYPAL RENEWAL INITIATED - CURRENCY:', currency, '=====');

      showNotification('💳 Initializing PayPal payment...', 'info');
      ...
      console.log('[renewal] Step 2: Processing PayPal payment...');
      showNotification('💳 Redirecting to PayPal...', 'info');
      
      // PayPal configuration based on currency
      const paypalConfig = {
        USD: {
          amount: '49',
          currency: 'USD',
      ...
      // PayPal.me URL (works with personal accounts)
      // Replace 'shank122004' with your PayPal.me username
      const paypalMeUrl = `https://www.paypal.me/shank122004/${config.amount}${config.currency}?description=RestaurantOS Monthly Subscription (${currency})`;
      
      console.log('[renewal] Building PayPal.me URL for', currency, 'with amount:', config.amount);
      ...
      console.log('[renewal] Redirecting to PayPal.me...');
      console.log('[renewal] Amount:', config.amount, config.currency);
      console.log('[renewal] Payment Description: RestaurantOS Monthly Subscription');

      // Redirect to PayPal.me checkout
      window.location.href = paypalMeUrl;

    } catch (error) {
      console.error('[renewal] Unexpected PayPal error:', error);
      showNotification('❌ PayPal payment failed: ' + (error.message || 'Unknown error'), 'error');
    }
  };

  console.log('[renewal-payment.js] Module loaded and ready with multi-currency PayPal support');
