...
// ============================================
// SETUP FEE PAYMENT ORDER - Cashfree PG v2023-08-01
// ============================================

exports.setupFeePaymentOrder = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
  res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
  ...
  try {
    console.log('[setupFeePaymentOrder] Request received');

    if (!CASHFREE_APP_ID || !CASHFREE_SECRET_KEY) {
      console.error('[setupFeePaymentOrder] Credentials missing');
      return res.status(500).json({
        success: false,
        error: "Cashfree credentials not configured"
      });
    ...
    if (!ownerId || !userEmail) {
      console.error('[setupFeePaymentOrder] Missing required fields');
      return res.status(400).json({
        success: false,
        error: "Missing required fields: ownerId, userEmail"
      });
    }

    console.log('[setupFeePaymentOrder] Creating order for:', userEmail);
    
    // Get restaurant data to determine country and setup fee
    ...
    // Determine setup fee amount based on country
    let setupFeeAmount, currency;
    if (country === 'USA') {
      setupFeeAmount = 25;
      currency = 'USD';
    } else if (country === 'UK') {
    ...
    const paymentData = {
      order_id: orderId,
      order_amount: setupFeeAmount,
      order_currency: currency,
      customer_details: {
        customer_id: ownerId,
        customer_email: userEmail,
        customer_phone: restaurantData.phone || ''
      },
      order_meta: {
        return_url: `${APP_URL}?setupFeePaymentComplete=${orderId}`,
        notify_url: `${process.env.NOTIFY_URL || ''}`
      },
      order_note: `Setup Fee for ${restaurantData.name || 'Restaurant'}`
    };

    console.log('[setupFeePaymentOrder] Creating Cashfree session...');
    const cashfreeResponse = await axios.post(
      `${CASHFREE_API_URL}/pg/orders`,
      paymentData,
      {
    ...
    if (!cashfreeResponse.data || !cashfreeResponse.data.payment_session_id) {
      console.error('[setupFeePaymentOrder] Invalid Cashfree response');
      return res.status(500).json({
        success: false,
    ...
    console.log('[setupFeePaymentOrder] Order created successfully:', orderId);

    // Store setup fee payment record
    await db.collection('setup_fee_payments').doc(orderId).set({
      orderId: orderId,
      ownerId: ownerId,
      userEmail: userEmail,
    ...
    console.error('[setupFeePaymentOrder] Error:', error.message);
    return res.status(500).json({
      success: false,
      error: "Failed to create setup fee payment order",
      details: process.env.NODE_ENV === 'development' ? error.message : undefined
    });
  }
});
...
exports.verifySetupFeePayment = functions.https.onRequest(async (req, res) => {
  res.set("Access-Control-Allow-Origin", "*");
  res.set("Access-Control-Allow-Methods", "POST, OPTIONS");
  res.set("Access-Control-Allow-Headers", "Content-Type, Authorization");
  ...
  try {
    ...
    console.log('[verifySetupFeePayment] Verifying order:', orderId);

    // Get the payment record
    const paymentRef = db.collection('setup_fee_payments').doc(orderId);
    ...
    console.log('[verifySetupFeePayment] Order status:', orderStatus);
    ...
    if (orderStatus === 'PAID') {
      ...
      // Mark setup fee as paid in restaurant document
      const restaurantRef = db.collection('restaurants').doc(paymentData.restaurantId);
      await restaurantRef.update({
        'setupFee.paid': true,
        'setupFee.paidAt': admin.firestore.FieldValue.serverTimestamp(),
        'setupFee.amount': paymentData.amount,
        'setupFee.currency': paymentData.currency,
        'setupFee.orderId': orderId
      });

      console.log('[verifySetupFeePayment] Setup fee marked as paid for:', paymentData.userEmail);

      return res.status(200).json({
        success: true,
        message: "Setup fee payment verified successfully",
        orderStatus: "PAID"
      });
      ...
      await paymentRef.update({
        ...
        paymentSessionStatus: orderStatus
      });
    ...
    console.error('[verifySetupFeePayment] Error:', error.message);
    return res.status(500).json({
      success: false,
